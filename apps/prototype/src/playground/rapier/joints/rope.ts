...
type CreateRopeAnchorOptions = {
  anchorOffset?: { bob?: Vector3Like; ceiling?: Vector3Like };
...
/**
 * A rope joint in Rapier prevents two anchors from getting farther apart than
 * length (it only “pulls” when stretched; it does not push).
 */
function createRopeAnchor(
  Rapier: typeof RapierNS,
  world: RapierNS.World,
  scene: Scene,
  {
    ...
    material,
    rope,
  }: CreateRopeAnchorOptions
): RopeAnchor {
  const {
    bob: anchorBobOffset = { x: 0, y: 0, z: 0 },
    ceiling: anchorCeilingOffset = { x: 0, y: 0, z: 0 },
  } = anchorOffset;

  const ceilingPosition = new Vector3().copy(ceilingBody.translation());
  const bobPosition = new Vector3().copy(bobBody.translation());
  const bobRotation = new Quaternion().copy(bobBody.rotation());

  const anchorCeilingLocalPosition = new Vector3().copy(anchorCeilingOffset);
  const anchorCeilingWorldPosition = new Vector3().addVectors(
    ceilingPosition,
    anchorCeilingOffset
  );

  const anchorBobLocalPosition = new Vector3().copy(anchorBobOffset);
  const anchorBobWorldPosition = new Vector3().addVectors(
    bobPosition,
    anchorBobOffset
  );
  const anchorBobLength = anchorBobWorldPosition.distanceTo(bobPosition);

  const joint = world.createImpulseJoint(
    Rapier.JointData.rope(
      length,
      anchorCeilingLocalPosition,
      anchorBobLocalPosition
    ),
    ceilingBody,
    bobBody,
    true
  ...
  const mesh = new Mesh(
    createBasicRopeGeometry({
      points: [
        ceilingPosition,
        anchorCeilingWorldPosition,
        anchorBobWorldPosition,
        bobPosition,
      ],
      ...rope,
    }),
  ...
  const update = () => {
    ...
    bobRotation.copy(bobBody.rotation());

    anchorBobWorldPosition.copy(
      addScaledQuaternion(bobPosition, bobRotation, anchorBobLength)
    );
...
const BOB_RADIUS = 0.8;
const ANCHOR_BOB_OFFSET = { x: 0, y: BOB_RADIUS + 0.1, z: 0 };
const ANCHOR_CEILING_OFFSET = { x: 0, y: -0.1, z: 0 };
const MAX_IMPULSE_MAGNITUDE = 5;
const MIN_IMPULSE_MAGNITUDE = 1;
...
export async function run() {
  ...
  const impulseHelper = new ImpulseHelper({
    maxImpulseMagnitude: MAX_IMPULSE_MAGNITUDE,
    minImpulseMagnitude: MIN_IMPULSE_MAGNITUDE,
  });
  impulseHelper.visible = controls.physics.helper ?? false;

  const physicsWorldHelper = createPhysicsWorldHelper(world);
  physicsWorldHelper.lines.visible = controls.physics.helper ?? false;

  scene.add(impulseHelper, physicsWorldHelper.lines);
  ...
  const distanceBetweenAnchors = new Vector3()
    .addVectors(ceiling.body.translation(), ANCHOR_CEILING_OFFSET)
    .distanceTo(
      new Vector3().addVectors(bob.body.translation(), ANCHOR_BOB_OFFSET)
    );

  const ropeAnchor = createRopeAnchor(Rapier, world, scene, {
    anchorOffset: { bob: ANCHOR_BOB_OFFSET, ceiling: ANCHOR_CEILING_OFFSET },
    bobBody: bob.body,
    ceilingBody: ceiling.body,
    length: distanceBetweenAnchors * 4,
  ...
  const handleBobClick = (
    intersection: Intersection<Mesh<SphereGeometry, Material>>
  ) => {
    const point = intersection.point;
    const multiplier = lerp(
      Math.random(),
      MIN_IMPULSE_MAGNITUDE,
      MAX_IMPULSE_MAGNITUDE
    );

    const impulse = bob.mesh.position
      .clone()
      .sub(point)
      .normalize()
      .multiplyScalar(multiplier);

    impulseHelper.addImpulsePoint(impulse, point);
    bob.body.applyImpulseAtPoint(impulse, point, true);
  };
  ...
  const clickEventSub = fromEvent<PointerEvent>(
    ...
    if (currentIntersection && currentIntersection.object === bob.mesh) {
      handleBobClick(
        currentIntersection as Intersection<Mesh<SphereGeometry, Material>>
      );
    }
  });
