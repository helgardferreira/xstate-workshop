  ...
  public override updateMatrixWorld = (): void => {
    ...
    for (let i = 0; i < handles.length; i++) {
      ...
      if (this.mode === 'translate' || this.mode === 'scale') {
        ...
        // Flip translate and scale axis occluded behind another axis
        ...
        if (handle.name.search('X') !== -1) {
          if (
            this.alignVector
              .copy(this.unitX)
              .applyQuaternion(quaternion)
              .dot(this.eye) < AXIS_FLIP_THRESHOLD
          ) {
            if (handle.tag === 'fwd') {
              handle.visible = false;
            } else {
      ...
      //@ts-expect-error ignore
      handle.material.tempOpacity =
        (
          (handle as Mesh).material as (
            | MeshBasicMaterial
            | LineBasicMaterial
          ) & { tempOpacity: number | undefined }
        ).tempOpacity ||
        ((handle as Mesh).material as MeshBasicMaterial | LineBasicMaterial)
          .opacity;
      //@ts-expect-error ignore
